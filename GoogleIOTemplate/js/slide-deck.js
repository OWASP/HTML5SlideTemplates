...
SlideDeck.prototype.onDomLoaded_ = function(e) {
  ...
  var urlify = this.container.querySelectorAll('.urlify');
  for (var i = 0; i < urlify.length; i++) {
      var a = document.createElement("a");
      a.href = urlify[i].textContent;
      a.textContent = urlify[i].textContent;
      urlify[i].innerHTML = "";
      urlify[i].appendChild(a);
  }
...
SlideDeck.prototype.updateHash_ = function(dontPush) {
  if (!dontPush) {
    ...
    var hash = '#' + slideNo;
    if (window.slidedeck) {
      if (window.slidedeck.controller.popup) {
        window.slidedeck.controller.popup.slidedeck.loadSlide(slideNo);
      }
    }
